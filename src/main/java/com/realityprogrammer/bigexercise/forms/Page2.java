...
public class Page2 implements Initializable {
    @FXML
    private FlowPane modificationButtonPane;
    ...
    @FXML private Slider addPositionSlider;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (!ApplicationData.getStudentList().isEmpty()) {
            addPositionSlider.setDisable(false);
            addPositionSlider.setMax(ApplicationData.getStudentList().size());
        }
    }
        ...
        if (id.isEmpty() || id.equals("0")) {
            ...
            try {
                ...
                if (avgStore >= 0 && avgStore <= 10) {
                    ApplicationData.getStudentList().addAt(new Student(id, name, avgStore), (int)addPositionSlider.getValue());
